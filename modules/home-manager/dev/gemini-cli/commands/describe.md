...
Current jujutsu status: !`jj status --no-pager`
...
1. **Analyze the current changes** by running `jj diff --no-pager -r $ARGUMENTS --git` (where $ARGUMENTS is the revset, defaulting to `@`)
...
4. **Apply the description** by running `jj describe --no-pager -m "your generated message" $ARGUMENTS`
