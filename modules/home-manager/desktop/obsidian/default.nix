...

{
  ...
  config = lib.mkIf config.home-manager.desktop.obsidian.enable (
    let
      obsidianSkills = libEx.loadSkills flake.inputs.obsidian-skills;
      corePlugins = [
        "backlink"
        "bases"
        "bookmarks"
      ...
      vaultSkillFiles = lib.concatMapAttrs (
        _: vault:
        lib.mapAttrs' (name: source: {
          name = "${vault.target}/.agents/skills/${name}";
          value = { inherit source; };
        }) obsidianSkills
      ) vaults;
      syncVaultRels = map (lib.removePrefix "Sync/") (
        lib.filter (lib.hasPrefix "Sync/") (map (vault: vault.target) (lib.attrValues vaults))
      );
      stignoreText = lib.concatMapStrings (rel: "${rel}/.agents\n${rel}/.pi\n") syncVaultRels;
    in
    {
      home.file =
        vaultSkillFiles
        // lib.optionalAttrs (stignoreText != "") {
          "Sync/.stignore".text = stignoreText;
        };
