...
(define-configuration nyxt/mode/proxy:proxy-mode
  ((nyxt/mode/proxy:proxy (make-instance 'proxy
                                         :url (quri:uri "socks5://localhost:7890")
                                         :allowlist '("localhost")
  ...
  (when (slot-boundp buffer 'nyxt/renderer/gtk::gtk-object)
    (let* ((settings (webkit:webkit-web-view-get-settings
      ...
      (setf
       ...
       ;; fonts
       (webkit:webkit-settings-default-font-family settings) "Roboto"
       (webkit:webkit-settings-default-font-size settings) 15
       (webkit:webkit-settings-monospace-font-family settings) "Hack Nerd Font Mono"
