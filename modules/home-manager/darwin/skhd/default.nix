...
{
  options.home-manager.darwin.skhd.enable = lib.mkEnableOption "skhd config" // {
    default = config.home-manager.darwin.yabai.enable;
  };

  config = lib.mkIf config.home-manager.darwin.skhd.enable {
    home.packages = with pkgs; [ skhd ];
    xdg.configFile."skhd/skhdrc".source = pkgs.writeTextFile {
      name = "skhdrc";
      text =
        let
        ...
        ''
          ...
          :: stack   @         : ${lib.getExe modeController} stack # stack mode: interact with stacks
          :: display @         : ${lib.getExe modeController} display # display mode: focus displays, move windows to other displays
          :: window  @         : ${lib.getExe modeController} window # window mode: manipulate windows and spaces
          :: resize  @         : ${lib.getExe modeController} resize # resize mode: resize windows in current space
          :: inst    @         : ${lib.getExe modeController} inst # insert mode: tell yabai where to insert the new window
          :: reload  @         : ${lib.getExe modeController} reload # reload mode: restart services
          ...
          ctrl + shift - z      : ${lib.getExe toggleZenMode}; \
                                  skhd -k 'escape'
