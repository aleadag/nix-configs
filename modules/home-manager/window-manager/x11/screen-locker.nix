...
{
  options.home-manager.window-manager.x11.screen-locker.enable =
    lib.mkEnableOption "screen-locker config"
    // {
      default = config.home-manager.window-manager.x11.enable;
    };

  config = lib.mkIf config.home-manager.window-manager.x11.screen-locker.enable {
    services.screen-locker = {
      ...
      lockCmd = toString (
        pkgs.writeShellScript "lock-screen" ''
          ...
          export XSECURELOCK_FONT="Roboto:style=Regular"
