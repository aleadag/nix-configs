...
Use the distro-provided `swaylock` so it links against the host PAM
implementation and can read the host PAM configuration syntax:

```console
$ sudo apt install swaylock
```

Set `programs.swaylock.package = null;` in Home Manager for those hosts. The
Wayland swayidle module will then call `/usr/bin/swaylock` instead of a Nix
store path.
...
Point `nix-daemon` directly to Home Manager's SOPS-generated credentials file
(root can read user files directly, avoiding duplicating secrets outside HM).

Add a systemd drop-in for `nix-daemon`:

```ini
# /etc/systemd/system/nix-daemon.service.d/aws-credentials.conf
[Service]
Environment=AWS_SHARED_CREDENTIALS_FILE=/home/<username>/.config/niks3/aws_credentials
```
